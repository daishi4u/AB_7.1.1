...
#include <trace/events/cpufreq_interactive_pro.h>
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	if (pcpu->policy->cur >= this_hispeed_freq &&
		...
		trace_cpufreq_interactive_pro_notyet(
	...
	if (new_freq < pcpu->target_freq &&
		...
		trace_cpufreq_interactive_pro_notyet(data, cpu_load,
	...
	if (pcpu->target_freq == new_freq &&
		...
		trace_cpufreq_interactive_pro_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_interactive_pro_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
static int cpufreq_interactive_speedchange_task(void *data)
{
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			trace_cpufreq_interactive_pro_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_interactive_pro_boost("on");
		cpufreq_interactive_boost();
	} else {
		boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_interactive_pro_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_interactive_pro_boost("pulse");
	cpufreq_interactive_boost();
	return count;
}
...
static struct attribute *interactive_pro_attributes[] = {
...
static struct attribute_group interactive_pro_attr_group = {
	.attrs = interactive_pro_attributes,
	.name = "interactivepro",
};
...
static
#endif
struct cpufreq_governor cpufreq_gov_interactivepro = {
	.name = "interactivepro",
...
static int __init cpufreq_interactive_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_interactivepro);
}
...
{
	cpufreq_unregister_governor(&cpufreq_gov_interactivepro);
