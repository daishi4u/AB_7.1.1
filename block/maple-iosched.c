...
maple_dispatch_requests(struct request_queue *q, int force)
{
	...
	if (!rq) {
		...
		if (power_suspend_active && mdata->starved >= mdata->writes_starved)
			data_dir = WRITE;
		else if (!power_suspend_active && mdata->starved >= 1)
			data_dir = WRITE;
